...
func newGlabDocsCommand(run func(cmd *cobra.Command, args []string)) (*cobra.Command, error) {
	...
	command.PersistentFlags().String("component-prefix", "", "component address used in the generated usage snippet, e.g. gitlab.com/my-group/my-project. When empty, resolves to $CI_SERVER_FQDN/<project-path>, with <project-path> from $CI_PROJECT_PATH or, failing that, the local git remote")
	command.PersistentFlags().String("gitlab-server-url", "", "GitLab server base URL, e.g. https://gitlab.com, used to resolve a literal $CI_SERVER_FQDN in `component:` include addresses in the Includes table and link them to their source project. When empty, $CI_SERVER_FQDN is left as-is and no link is added")
	command.PersistentFlags().Bool("include-details", true, "fill the Includes table's Description column with the variables and jobs each include adds, reading `local:` includes from the checkout and fetching `project:`/`component:` includes through the GitLab API of --gitlab-server-url (following nested includes)")
	command.PersistentFlags().String("gitlab-token", "", "GitLab token (read_api / read_repository) sent as PRIVATE-TOKEN when fetching `project:`/`component:` includes for --include-details. When empty, $CI_JOB_TOKEN is sent as JOB-TOKEN instead, if set. Prefer the GLAB_DOCS_GITLAB_TOKEN env var over the flag")
